...
class BaseLogger(object):
    def __init__(self, logger_args, start_epoch, num_epochs, batch_size, dataset_len, device, is_training=False, normalization='imagenet'):
        ...
        elif normalization == 'cxr_pulm_tb_norm':
            self.un_normalize = util.UnNormalize(mean=CXR_PULM_TB_MEAN, std=CXR_PULM_TB_STD)
    ...
    def visualize(self, inputs, logits, targets, phase, unique_id=None):
        ...
        cls_logits = logits.detach().to('cpu')
        ...
        for i in range(self.num_visuals):
            ...
            #TODO (amit): generalize to multitask.
            label = targets[i].data[0]

            # Log to TensorBoard
            tag = '{}/label_{}/prob{}_{:.4f}'.format(phase, (i+1)
                if unique_id is None else unique_id, label, cls_probs[i][0])
