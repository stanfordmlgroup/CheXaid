...
from dataset.constants import CXR_PULM_TB_MEAN, CXR_PULM_TB_STD, IMAGENET_MEAN, IMAGENET_STD
...
class BaseDataset(Dataset):
    ...
    def _set_transforms(self, t_args):
       ...
       # Normalization
        if t_args.clahe:
            transforms_list += [CLAHE(
                clip_limit=1.0, tile_grid_size=(16, 16))]

        # Data augmentation
        if self.is_training:
            transforms_list += [
                transforms.RandomAffine(
                degrees=t_args.transform_affine_all, # number of degrees
                translate=((t_args.transform_affine_all / 100),
                    (t_args.transform_affine_all / 100)), # from degrees to proportion of image
                scale=(1 - (t_args.transform_affine_all / 100),
                    1 + (t_args.transform_affine_all / 100)), # from degrees to proportion of image
                shear=0, # number of degrees
                fillcolor=0) if t_args.transform_affine_all else None,
                transforms.RandomHorizontalFlip() if t_args.horizontal_flip else None
            ]
        ...
        elif t_args.normalization == 'cxr_pulm_tb_norm':
            normalize = transforms.Normalize(mean=CXR_PULM_TB_MEAN, std=CXR_PULM_TB_STD)
